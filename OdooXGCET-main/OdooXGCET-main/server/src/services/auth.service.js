...
// Get all employees with their user account details (HR only - full details)
const getAllEmployees = async () => {
  try {
    // Get all employees
    const employees = await Employee.find().sort({ createdAt: -1 });

    // Get all users and populate employee details
    const users = await User.find()
      .populate('employeeId')
      .select('-passwordHash')
      .sort({ createdAt: -1 });

    // Create a map of employeeId to user for quick lookup
    const userMap = new Map();
    users.forEach(user => {
      if (user.employeeId) {
        userMap.set(user.employeeId._id.toString(), {
          _id: user._id,
          loginId: user.loginId,
          role: user.role,
          isFirstLogin: user.isFirstLogin,
    ...
    // Combine employee data with user account information
    const employeesWithDetails = employees.map(employee => {
      const userAccount = userMap.get(employee._id.toString()) || null;
...
// Get all employees (view only - accessible to all authenticated users)
const getAllEmployeesView = async () => {
  try {
    // Get all employees with basic information only
    const employees = await Employee.find()
      .select('employeeCode firstName lastName companyName status createdAt')
      .sort({ createdAt: -1 });

    // Get all users for role information only
    const users = await User.find()
      .populate('employeeId', '_id')
      .select('employeeId role')
      .sort({ createdAt: -1 });

    // Create a map of employeeId to role for quick lookup
    const roleMap = new Map();
    users.forEach(user => {
      if (user.employeeId) {
        roleMap.set(user.employeeId._id.toString(), user.role);
      }
    });

    // Combine employee data with role information only (no sensitive data)
    const employeesList = employees.map(employee => {
      const role = roleMap.get(employee._id.toString()) || null;
